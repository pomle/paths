  ...
  describe('#parser', () => {
    ...
    describe('#matchPath', () => {
      ...
      it('returns positive integer if path starts with parser', () => {
        ...
        expect(
          parser.matchPath('/my/foo/parsable/url/bar/and/four/more/parts'),
        ).toBe(4);
      });

      it('returns count of missing parts when static part is mismatching', () => {
        expect(parser.matchPath('/your/foo/parsable/url/bar')).toBe(-5);
        expect(parser.matchPath('/my/foo/nonparsable/url/bar')).toBe(-3);
      });
